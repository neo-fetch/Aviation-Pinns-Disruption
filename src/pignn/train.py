...
def train_model(g: GraphTensors, train_ds: WindowDataset,
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        # validation (prediction loss only, comparable across models)
        model.eval()
        with torch.no_grad():
        ...
        history.append({"epoch": epoch, "train_loss": ep_loss / max(n_b, 1),
                        "val_loss": float(val_loss), "lambda": lam})
        if verbose and (epoch % 10 == 0 or epoch == cfg.EPOCHS - 1):
            print(f"  epoch {epoch:3d}  train {ep_loss / max(n_b, 1):.4f}  "
                  f"val {float(val_loss):.4f}  lam {lam:.3f}")

        # early stopping only after curriculum is mostly ramped in
        ramped = (not physics) or epoch >= cfg.WARMUP_EPOCHS + cfg.CURRICULUM_TAU
        if ramped:
            if float(val_loss) < best_val - 1e-4:
                best_val, patience = float(val_loss), 0
